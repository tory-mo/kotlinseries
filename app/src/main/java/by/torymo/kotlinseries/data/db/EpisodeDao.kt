...
interface EpisodeDao {
    ...
    @Query("SELECT episodes.*, series.name as seriesName, seasons.number as seasonNumber from episodes inner join series on series.id=episodes.seriesId inner join seasons on seasons.id = episodes.seasonId WHERE episodes.date = :date order by episodes.date asc")
    fun getEpisodesForDate(date: Long): List<ExtendedEpisode>

    @Query("SELECT episodes.*, series.name as seriesName, seasons.number as seasonNumber from episodes inner join series on series.id=episodes.seriesId inner join seasons on seasons.id = episodes.seasonId WHERE (seen = 0) and (episodes.date = :date) order by episodes.date asc")
